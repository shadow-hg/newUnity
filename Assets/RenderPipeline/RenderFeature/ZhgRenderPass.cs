...
public class ZhgRenderPass : ScriptableRenderPass
{
    ...
    public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
    {
        ...
        if (m_zhgVolumeSetting.IsActive() && !renderingData.cameraData.isSceneViewCamera)
        {
            ...
            {
                UnityEngine.Debug.LogWarning(materialProperties.Count + "--" + materialAnimations.Count +
                                             "材质属性和动画曲线数量不对应！");
                return;
            }
            ...
            if (!enableDebug)
            {
                ...
                {
                    ScreenMaterial.SetFloat(materialProperties[i],
                        materialAnimations[i].Evaluate(animTime) * animSpeed);
                }
            }
    ...
    public void Setup(RenderTargetIdentifier colorAttachment, Material material,
        List<AnimationCurve> materialAnimations, List<string> materialProperties, bool EnablePlay, float animSpeed)
    {
